...
int main(int argc, char** argv){
	...
	//Adds subscriber that subscribes left slider value on appropriate topic, reads with callback
	...
	while(ros::ok()){
		...
		if (walker_mode.data == manual_free_roll) {
			//vel_cmd_out.data = free_roll_flag; //Identifies free roll to motor controller
			vel_cmd_out.data = 0; //for the demo
		...
		} else if (walker_mode.data == auto_go_user) {
			//vel_cmd_out = vel_cmd_in;
			vel_cmd_out.data = free_roll_flag;	//for the demo
		} else if (walker_mode.data == auto_go_park) {
			vel_cmd_out.data = -free_roll_flag;	//for the demo
		}
		else if (walker_mode.data == auto_stop) {
			vel_cmd_out.data = 0;	//for the demo
		}
